...
| Markets: Gold, Crypto, NEPSE | `/markets` `/gold` `/crypto` `/nepse` | FENEGOSIDA + GoldAPI + NRB, CoinGecko, your NEPSE provider |
...
| `GOLDAPI_KEY` | International gold/silver spot | goldapi.io (optional) |
| `NEPAL_GOLD_JSON_URL` | Nepal gold from a licensed JSON source instead of parsing FENEGOSIDA | optional |
...
`api/news.js`, `football.js`, `cricket.js`, `f1.js`, `gold.js` and `nepse.js` are thin wrappers over `api/_lib/handlers.js`.
...
Nepal rates are parsed from FENEGOSIDA's public page with strict sanity ranges. If the page layout changes,
the parser fails closed ("unavailable") rather than showing a wrong price. The page also shows the international
spot price and a clearly labelled reference conversion at NRB's USD rate that excludes duty, VAT and margins.
...
gold and NEPSE show "not connected" unless you deploy the proxy separately (e.g. on Vercel) and set `VITE_LIVE_API_BASE`
