...
import { WHATSAPP, whatsappLink } from '../../data/site';
...
export function WhatsAppButton({ children = 'Chat with us on WhatsApp', message, className = '', size = 'lg' }) {
  return (
    <a className={`btn btn-wa btn-${size} ${className}`} href={whatsappLink(message)} target="_blank" rel="noopener noreferrer">
...
export function WhatsAppFloat() {
  ...
  return (
    <a className={`wa-float${hidden ? ' is-hidden' : ''}`} href={whatsappLink()} target="_blank" rel="noopener noreferrer"
    ...
    <section className="wa-section" data-wa-section aria-labelledby="wa-title">
      <div className="container wa-inner">
        ...
        <div className="wa-actions">
          <WhatsAppButton>Chat with HimalayaHub</WhatsAppButton>
          <p className="wa-number">WhatsApp <a href={`tel:+977${WHATSAPP.local}`}>{WHATSAPP.local}</a></p>
        </div>
      </div>
    </section>
  );
