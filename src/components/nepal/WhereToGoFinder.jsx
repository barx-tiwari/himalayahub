...
import { EXPERIENCES, destinations, getProvince } from '../../data/nepalPlaces';
...
export default function WhereToGoFinder({ feed }) {
  ...
  const list = exp ? destinations.filter((d) => matches(exp, d, all[d.id])) : destinations.filter((d) => d.featured).slice(0, 5);
  return (
