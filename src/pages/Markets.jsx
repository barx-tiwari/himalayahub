...
export function useGold() { return useLiveDataRefresh('gold', getMetals, { interval: REFRESH.gold.base }); }
  ...
  return (
    <div className="container page" data-domain="markets">
      <header className="page-header">
        <h1>Markets</h1>
        <p>Gold and silver in Nepal and internationally, major cryptocurrencies and the Nepal Stock Exchange — each with its source and update time.</p>
      </header>
      <Disclaimer>{FINANCE_DISCLAIMER}</Disclaimer>
      <div className="hub-grid section" style={{ marginTop: 20 }}>
        <section className="card hub-tile" aria-labelledby="m-gold">
          <h2 id="m-gold">Gold &amp; Silver</h2>
          <DataState feed={gold} compact>{(d) => (d.nepal?.fineGoldTola ? <><span className="price-big">Rs {d.nepal.fineGoldTola.toLocaleString()}</span><span className="small muted">Fine gold per tola (FENEGOSIDA)</span></> : <p className="small muted" style={{ margin: 0 }}>Nepal rate currently unavailable.</p>)}</DataState>
          <LiveDataRefresh feed={gold} compact />
          <Link to="/gold" className="btn btn-secondary btn-sm">View gold prices</Link>
        </section>
